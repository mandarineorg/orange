...
export const Table = (data: unknown, inString: boolean, useQuotes: boolean, customIndexKey?: "none" | string, properties?: string[]): void | string => {
    ...
    let indexKey = isSet || isMap ? "(iter idx)" : "(idx)";
    if(customIndexKey && customIndexKey != "none") indexKey = customIndexKey;
    ...
    let body = [];
    let header = [];
    if(customIndexKey && customIndexKey != "none") {
      header = [
        indexKey,
        ...(properties || [...headerKeys, !isMap && hasPrimitives && valuesKey]),
      ].filter(Boolean) as string[];
      body = [indexKeys, ...bodyValues, values]
    } else {
      header = [
        ...(properties || [...headerKeys, !isMap && hasPrimitives && valuesKey])
      ].filter(Boolean) as string[];
      body = [...bodyValues, values]
    }

    return toTable(header, body);
};
